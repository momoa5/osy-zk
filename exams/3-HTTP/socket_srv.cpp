...
int main( int t_narg, char **t_args )
{
    ...
    sem = sem_open("child_semaphore", O_CREAT, 0660, 1);
    sem_init(sem, 1, 1);
        ...
        if(child == 0)
        {
            ...
            { // communication
                ...
                if ( l_read_poll[ 1 ].revents & POLLIN )
                {
                    ...
                    sscanf(l_buf, "GET /%s HTTP", &buffer);
                    ...
                    int len_l = strlen(prikaz[i - 1]);
                    if (prikaz[i - 1][len_l - 1] == '\n')
                        prikaz[i - 1][len_l - 1] = 0;
                    prikaz[i] = NULL;

                    write(STDOUT_FILENO, prikaz, strlen(*prikaz));
                    ...
                    write(l_sock_client, head, strlen(head));
